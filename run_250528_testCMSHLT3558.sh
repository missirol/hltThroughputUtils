...
hltConfigFromDB --configName --adg /cdaq/test/missirol/dev/CMSSW_15_0_0/CMSHLT_3558/Test01/HLT/V1 > tmp_baseline.py

https_proxy=http://cmsproxy.cms:3128/ \
hltConfigFromDB --configName --adg /cdaq/test/missirol/dev/CMSSW_15_0_0/CMSHLT_3558/Test01/HLT/V2 > tmp_withAXOL1TL.py

https_proxy=http://cmsproxy.cms:3128/ \
hltConfigFromDB --configName --adg /cdaq/test/missirol/dev/CMSSW_15_0_0/CMSHLT_3558/Test01/HLT/V3 > tmp_withAXOL1TL_withCICADA.py

cp /gpu_data/store/data/Run2025*/EphemeralHLTPhysics/FED/run"${runNumber}"_cff.py .

for jobSubLabel in baseline withAXOL1TL withAXOL1TL_withCICADA; do
...
process = customizeHLTforCMSHLT3558(process)
@EOF
