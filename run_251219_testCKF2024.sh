...
hltConfigFromDB --runNumber 398802 > tmp0.py
...
for jobSubLabel in baseline target1 target2; do
  ...
  # For "CKF2024", use a single SiStrips unpacker in the HLT configuration
  if [ "${jobSubLabel}" != baseline ]; then
    sed -i "s|hltSiStripRawToClustersFacilityOnDemand|hltSiStripRawToClustersFacility|g" tmp.py
    sed -i "s|hltMeasurementTrackerEventOnDemand|hltMeasurementTrackerEvent|g" tmp.py
    sed -i "s|hltMeasurementTrackerEventOnDemandSerialSync|hltMeasurementTrackerEventSerialSync|g" tmp.py
    sed -i "s|HLTDoLocalStripOnDemandSequence|HLTDoLocalStripSequence|g" tmp.py
    sed -i "s|HLTDoLocalStripOnDemandSequenceSerialSync|HLTDoLocalStripSequenceSerialSync|g" tmp.py
  fi
...
process = customizeHLTforTestCKF2024_${jobSubLabel}(process)
@EOF
