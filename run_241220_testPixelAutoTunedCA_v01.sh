...
hltConfigFromDB --configName --adg /cdaq/physics/Run2024/2e34/v1.4.3/HLT/V2 > tmp0.py

cp /gpu_data/store/data/Run2024*/EphemeralHLTPhysics/FED/run"${runNumber}"_cff.py .

for jobSubLabel in baseline newTuneV01; do
...
process = customizeHLTforPixelAutoTunedCA_${jobSubLabel}(process)
@EOF
