...
hltConfigFromDB --configName /users/missirol/test/dev/CMSSW_15_0_0/CMSHLT_3529/Test02/GRun/V1 > tmp_baseline.py

https_proxy=http://cmsproxy.cms:3128/ \
hltConfigFromDB --configName /users/missirol/test/dev/CMSSW_15_0_0/CMSHLT_3529/Test02/GRun/V2 > tmp_DiphotonOnly.py

cp /gpu_data/store/data/Run2024*/EphemeralHLTPhysics/FED/run"${runNumber}"_cff.py .

for jobSubLabel in baseline DiphotonOnly; do
...
process = customizeHLTforCMSHLT3529_baseline2(process)
@EOF
