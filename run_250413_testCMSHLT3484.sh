...
hltConfigFromDB --configName /users/missirol/test/dev/CMSSW_15_0_0/CMSHLT_3484/Test01/GRun/V1 > tmp_baseline.py

https_proxy=http://cmsproxy.cms:3128/ \
hltConfigFromDB --configName /users/missirol/test/dev/CMSSW_15_0_0/CMSHLT_3484/Test01/GRun/V2 > tmp_testL1HT200.py

cp /gpu_data/store/data/Run2024*/EphemeralHLTPhysics/FED/run"${runNumber}"_cff.py .

for jobSubLabel in baseline testL1HT200; do
...
process = customizeHLTforCMSHLT3484(process)
@EOF
