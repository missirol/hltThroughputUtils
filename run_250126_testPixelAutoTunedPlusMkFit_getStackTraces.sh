...
run() {
  ...
  foo=$(printf "%125s") && echo ${foo// /-} && unset foo
  rm -rf run"${runNumber}"
  ${2}/benchmark "${1}" -E 'cmsTraceExceptions cmsRun' -r 4 -j "${4}" -t "${5}" -s "${6}" -e 40100 -g 1 -n --no-cpu-affinity -l "${3}" -k resources.json --tmpdir "${outDir}"/tmp |& tee "${3}".log
...
for jobSubLabel in baseline pixel pixelAndMkFit; do
...
process = customizeHLTforPixelAutoTunedPlusMkFit_${jobSubLabel}(process)
@EOF
